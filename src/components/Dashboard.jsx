...
import { Users, TrendingUp, FileText, Briefcase, MessageCircle, CheckCircle2, Circle } from 'lucide-react'
    ...
    <div className="space-y-5 fade-in-up">
      ...
      <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-syne font-bold bg-gold-500/15 text-gold-500">Day 7 of 90</span>
        </div>
        <div className="h-3 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-gold-600 to-gold-400 rounded-full transition-all duration-1000" style={{ width: `${barWidth}%` }} />
        </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          { label: 'FOLLOWERS', value: followers.toLocaleString(), change: '+23 today', icon: Users },
          { label: 'ENGAGEMENT', value: `${metrics.engagement}%`, change: '+0.3%', icon: TrendingUp },
          { label: 'SCRIPTS', value: metrics.scriptsGenerated, change: 'Generated', icon: FileText },
          { label: 'PROSPECTS', value: brandProspects.length, change: 'In pipeline', icon: Briefcase },
          { label: 'DM INQUIRIES', value: metrics.dmInquiries, change: 'This week', icon: MessageCircle },
          ...
          <div key={label} className="bg-white/[0.03] border border-white/5 rounded-xl p-4 hover:border-gold-500/20 transition-colors">
            ...
            <p className="text-xs mt-1.5 font-dm text-emerald-400">{change}</p>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-dm text-zinc-500">{doneTasks}/{tasks ? tasks.length : 0} complete</span>
          </div>
          <div className="space-y-2">
            {tasks && tasks.map(task => (
              <button key={task.id} onClick={() => toggleTask(task.id)}
                ...
                {task.done ? <CheckCircle2 size={16} className="text-gold-500 flex-shrink-0 mt-0.5" /> : <Circle size={16} className="text-zinc-600 flex-shrink-0 mt-0.5" />}
                <span className={`text-xs font-dm flex-1 ${task.done ? 'line-through text-zinc-600' : 'text-zinc-300'}`}>{task.text}</span>
                {task.urgent && !task.done && <span className="text-[9px] font-syne font-bold bg-red-500/15 text-red-400 px-1.5 py-0.5 rounded-full">URGENT</span>}
              </button>
            ))}
          </div>
        </div>
          ...
          <table className="w-full text-xs font-dm">
            <thead>
              <tr className="text-zinc-600 text-left">
                {['Day','Format','Platform','Topic','Status'].map(h => <th key={h} className="pb-3 pr-4 font-semibold uppercase tracking-wider">{h}</th>)}
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04]">
              {CONTENT_PLAN.filter(d => d.day >= 5 && d.day <= 11).map(item => (
                <tr key={item.day} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="py-2.5"><span className={`inline-flex px-2 py-0.5 rounded-full font-semibold ${getStatusColor(item.day)}`}>{getStatusLabel(item.day)}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
