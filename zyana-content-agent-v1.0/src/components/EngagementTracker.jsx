...
export default function EngagementTracker() {
  ...
  const getAnalysis = async () => {
    setAnalyzing(true)
    const prompt = `Analyze these Zyana Systems content metrics and give 3 specific, actionable recommendations to improve growth.
Metrics: ${JSON.stringify({ ...metrics, ...form })}
Keep each recommendation under 2 sentences. Be specific and use numbers.`
  ...
  return (
    <div className="space-y-5 fade-in-up">
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[
              ...
              { key: 'emailSubs', label: 'Email Subs' },
              { key: 'dmInquiries', label: 'DM Inquiries' },
          ...
          <button onClick={saveMetrics}
            className="w-full bg-gradient-to-r from-gold-500 to-gold-600 text-black font-syne font-bold rounded-xl py-2.5 text-sm hover:opacity-90 transition-opacity">
        ...
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            {[
              { platform: 'Instagram', views: '1.8K', growth: '+12%' },
              { platform: 'TikTok', views: '3.2K', growth: '+28%' },
              { platform: 'YouTube', views: '680', growth: '+8%' },
            ].map(({ platform, views, growth }) => (
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
          <p className="font-syne font-semibold text-white mb-4">Views Per Video</p>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={VIEWS_DATA} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
              <XAxis dataKey="day" tick={{ fill: '#555', fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: '#555', fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="views" fill="#D4A843" radius={[4, 4, 0, 0]} name="Views" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
          <p className="font-syne font-semibold text-white mb-4">Engagement Rate %</p>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={ENG_DATA} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
              <XAxis dataKey="day" tick={{ fill: '#555', fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: '#555', fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="rate" fill="#D4A843" radius={[4, 4, 0, 0]} name="Engagement" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={getAnalysis} disabled={analyzing}
            className="flex items-center gap-2 bg-gradient-to-r from-gold-500 to-gold-600 text-black font-syne font-bold rounded-xl px-4 py-2 text-sm hover:opacity-90 transition-opacity disabled:opacity-50">
        ...
        {analysis ? (
          ...
          <p className="text-sm text-zinc-600 font-dm">Click "Get AI Analysis" to receive personalized growth recommendations based on your current metrics.</p>
        )}
      </div>
    </div>
  )
